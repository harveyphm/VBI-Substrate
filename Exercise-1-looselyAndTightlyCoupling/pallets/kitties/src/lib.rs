...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_price(
			...
			// Ensure the kitty exist and is called by the kitty owner
		...
		pub fn transfer( 
			...
			//ensure kitty exist and called by kitties owner
			ensure!(Self::is_kitty_owner(&kitty_id, &from)?, <Error<T>>::NotKittyOwner ); 

			//Verify the kiity is not transfering back to its owner
			ensure!(from != to, <Error<T>>::TransferToSelf); 

			//Verify the recipient has the capacity to receive one more kiity 
		...
		pub fn buy_kitty( 
			...
			//Tranfer the kitty from seller to buyer 
			Self::transfer_kitty_to(&kitty_id, &buyer); 
			//Deposit relevent event
	...
	// TODO Part IV: Write transfer_kitty_to
		...
		pub fn transfer_kitty_to( 
			...
			//Remove kitty id form the kittyOwner vector of prev_owner
